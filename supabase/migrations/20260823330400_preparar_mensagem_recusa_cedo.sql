-- =============================================================================
-- `preparar_mensagem` recusa cedo o que `registrar_abertura` recusaria depois
-- =============================================================================
-- ⚠️ Defeito de sequência, e dos que só aparecem no pior dia.
--
-- No painel, `abrirConversa()` fazia `window.open(...)` ANTES de esperar o
-- servidor:
--
--     window.open(mensagem.urlWhatsApp, JANELA_WA);   // abre primeiro
--     iniciar(async () => { const r = await registrarAbertura(...); ... });
--
-- Quando o servidor recusava — pessoa bloqueada, fora do horário, chip pausado,
-- DIA DA ELEIÇÃO — a janela do WhatsApp já estava aberta, com o texto da
-- campanha preenchido e o botão de enviar ao lado. A trava virava um aviso
-- vermelho no painel atrás, pedindo "não continue".
--
-- A correção tem duas frentes. A do navegador está em `atendimento.tsx` e
-- `perfil.tsx`: a janela abre em branco e só é apontada para o WhatsApp depois
-- do "ok" do servidor. Esta aqui é a outra metade — o texto nem chega a ser
-- montado quando o envio não pode acontecer.
--
-- Ficam de fora, de propósito, teto e intervalo: eles mudam sozinhos com o
-- passar dos minutos e recusar a MONTAGEM por causa deles esconderia do
-- atendente a mensagem que ele vai poder mandar daqui a um minuto. Esses dois
-- continuam sendo decididos na hora do envio, em `registrar_abertura`.
