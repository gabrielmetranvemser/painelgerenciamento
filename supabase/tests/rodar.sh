...
echo
"${PSQL[@]}" -f supabase/tests/27_quem_recebe_o_cadastro.sql 2>&1 | sed 's/^psql.*NOTICE:  //;s/^psql.*WARNING:  //' || falhou=1

echo
"${PSQL[@]}" -f supabase/tests/28_recepcao_no_whatsapp.sql 2>&1 | sed 's/^psql.*NOTICE:  //;s/^psql.*WARNING:  //' || falhou=1

echo
"${PSQL[@]}" -f supabase/tests/29_aparelhos_liberados.sql 2>&1 | sed 's/^psql.*NOTICE:  //;s/^psql.*WARNING:  //' || falhou=1

echo
"${PSQL[@]}" -f supabase/tests/30_desempenho_por_atendente.sql 2>&1 | sed 's/^psql.*NOTICE:  //;s/^psql.*WARNING:  //' || falhou=1
